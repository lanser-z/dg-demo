# column-level-lineage Specification

## Purpose

支持在 `lineage_recipe.yaml` 中声明列级血缘映射（`columnMappings`），并通过 `emit_lineage.py` 写入 DataHub GMS `upstreamLineage` aspect 的 `fineGrainedLineages` 字段，为 NL2SQL 提供字段溯源能力。
...
`scripts/emit_lineage.py` SHALL 在表级 `upstreamLineage` 写入基础上，对声明了 `columnMappings` 的 relationship 追加写入 `upstreamLineage` aspect 的 `fineGrainedLineages` 字段（DataHub v1.6 中列级血缘嵌套在 `upstreamLineage` 内，无独立 `upstreamColumnLineage` aspect）。写入 MUST 使用 `DatahubRestEmitter` 官方 SDK + `FineGrainedLineageClass`，MUST NOT 使用裸 `requests.post`。

#### Scenario: GMS 收到列级 aspect

- **WHEN** 执行 `uv run python scripts/emit_lineage.py` 且 recipe 含 columnMappings
- **THEN** GMS `upstreamLineage` aspect MUST 含 `fineGrainedLineages` 数组，每项含 downstream/upstream schemaField URN + `transformOperation`
