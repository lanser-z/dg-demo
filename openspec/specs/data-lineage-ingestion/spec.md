...
### Requirement: 列级血缘写入 upstreamLineage.fineGrainedLineages

`scripts/emit_lineage.py` SHALL 在表级 `upstreamLineage` 写入基础上，对 `lineage_recipe.yaml` 中声明了 `columnMappings` 的 relationship 追加写入 `upstreamLineage` aspect 的 `fineGrainedLineages` 字段（DataHub v1.6 中列级血缘嵌套在 `upstreamLineage` 内，无独立 `upstreamColumnLineage` aspect）。列级写入 MUST 使用 `DatahubRestEmitter` 官方 SDK + `FineGrainedLineageClass`，MUST NOT 使用裸 `requests.post`。
