# field-business-glossary Specification

## Purpose

为煤炭数据治理 Demo 的 12+ 张表提供字段级中文别名和业务术语映射，写入 DataHub `glossaryTerms` aspect，为 NL2SQL 智能问数提供术语消解能力。

## Requirements

### Requirement: 字段中文别名录入

系统 SHALL 通过 `scripts/emit_glossary.py` 为 12 张表的核心字段提供中文别名，写入 DataHub `glossaryTerms` aspect。词典数据 MUST 维护在 `data/glossary_terms.yaml` 中，字段包括 `platform`、`table`、`column`、`cn_name`、`description`、`business_terms`（业务术语列表）。
