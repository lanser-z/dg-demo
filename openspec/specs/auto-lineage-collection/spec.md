...
ETL 任务从 SQL 解析自动 emit OpenLineage 1.x 事件，HTTP 直发到 DataHub v1.6.0 GMS 的 OpenLineage REST 端点（`/openapi/openlineage/api/v1/lineage`），GMS 内部 `OpenLineageToDataHub` 转换器自动写 `upstreamLineage` aspect。覆盖 `LineageEmitter` 上下文管理器、sqlglot SQL 解析器、openlineage-python HTTP transport、端到端 lineage 验证与教学 notebook。
