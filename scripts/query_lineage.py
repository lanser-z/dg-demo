...
def query_upstream_lineage(urn: str) -> list:
    ...
    data = resp.json()
    # GMS v1.6 返回结构：{"version":0,"aspect":{"com.linkedin.dataset.UpstreamLineage":{...}}}
    # 兼容老版本：{"value":{...}} 或 {"<aspect>":{...}}
    aspect = data.get("aspect", {})
    if isinstance(aspect, dict):
        ul = aspect.get("com.linkedin.dataset.UpstreamLineage", {})
        if isinstance(ul, dict):
            upstreams = ul.get("upstreams", [])
